...
def resolve(env=None, pane_option_reader=read_pane_option, log=None):
    """Top-level resolver used by the hook / MCP / AUQ-hook. Pane-keyed only: the
    `@telegram_thread_id` pane option (authoritative) or the TELEGRAM_BRIDGE_THREAD_ID
    spawn env (the spawn-race window). Returns a thread id (str), or None when this
    is not a bridge session (the caller then abstains). Binding is now programmatic
    for every path — spawn script for /new + rollover, router for /attach — so the
    pre-pane cwd fallback is gone (the clean cutover); a pane with no option is, by
    definition, not bridged. The caller loads the registry entry by thread id for
    its inbox / session_dir."""
    return resolve_topic(env=env, pane_option_reader=pane_option_reader, log=log)


def _which_path():
    """Diagnostic: report how THIS pane/session resolves (pane / env / none).
    Run from a pane to see which binding signal wins."""
