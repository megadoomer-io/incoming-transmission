...
CONFIG_DEFAULTS = {"trigger_pct": 0.85, "warn_pct": 0.75, "kill_old": False,
                   ...
                   # to enable it. Detection is gated to MCP tool calls so a long
                   # Bash/native tool is never auto-cancelled. Dwell is a fixed 300s.
...
def _scan_outstanding_tool(transcript_path, json_loads=None):
    ...
    (outstanding_tool_id, outstanding_tool_name, last_progress_ts):
      ...
      outstanding_tool_name — the `name` of that outstanding tool_use (e.g.
        `mcp__gmail__send`, `Bash`), or None when there is no outstanding tool.
        The caller gates on this so only the MCP hang class is auto-cancelled.
    ...
    Robust to malformed lines (skipped) and missing files (returns
    (None, None, None)). json_loads is injectable for tests."""
    if json_loads is None:
        json_loads = json.loads
    last_tool_use_id = None          # newest assistant tool_use id seen
    last_tool_use_name = None        # its tool name (for the MCP-only gate)
    ...
    outstanding_name = last_tool_use_name if outstanding else None
    return (outstanding, outstanding_name, last_progress_ts)
...
def detect_hung_tool(reg, seen, now_mono, dwell_s,
    ...
    Detection is GATED to MCP tool calls (the outstanding tool_use name starts
    with `mcp__`) — a long Bash/WebFetch/native tool is never a candidate — and
    then requires: undrained inbox  +  outstanding MCP tool_use (no result)  +  no
    transcript progress since it first went outstanding, for >= dwell. A non-MCP
    outstanding tool or any missing signal clears the episode. scan/undrained are
    ...
    outstanding, outstanding_name, last_progress_ts = scan(tp)
    if not outstanding:
        seen.pop(tkey, None)                 # last tool_use was answered -> healthy
        return (None, seen)
    # MCP-only gate: the #18 hang class is an MCP tool that never returns (e.g. a
    # Gmail MCP wanting headless OAuth). A long Bash / WebFetch / native tool
    # legitimately runs with no transcript progress and an outstanding tool_use the
    # whole time, so without this gate an undrained inbox would Esc-cancel a HEALTHY
    # long task — exactly the wall-clock timeout #18 set out to avoid. Only flag MCP.
    if not (outstanding_name or "").startswith("mcp__"):
        seen.pop(tkey, None)
        return (None, seen)
    prev = seen.get(tkey)
...
def handle_hung_tool(reg, tkey, chat_id, dwell_s):
    ...
    same safe cancel as handle_wedge. Skips a topic with a permission OR an
    AskUserQuestion in flight (the hook / AUQ MCP own those) and a session whose
    pane can't be resolved."""
    global _hung_tool_state
    cpid = reg.get("claude_pid")
    thread_id = reg.get("thread_id")
    if not cpid or thread_id is None:
        return
    # A pending permission (perm-pending.json) or AskUserQuestion (auq-pending.json)
    # is a SUPPORTED outstanding tool call blocking on the owner — not a hang. The
    # AUQ MCP's own timeout (~1700s) far exceeds the hung-tool dwell (300s), so
    # without this skip an undrained inbox would Esc-cancel the owner's own question
    # ~23m early.
    sess_dir = INBOX_ROOT / tkey
    if (sess_dir / "perm-pending.json").exists() or (sess_dir / "auq-pending.json").exists():
...
def context_loop():
    ...
    while _running:
        ...
        # Hung-MCP-tool recovery (issue #18) — opt-in/off (owner-deferred). The
        # dwell default is a fixed 300s (CONFIG_DEFAULTS always supplies it, so the
        # old `wedge_dwell_seconds` fallback was dead code); override in
        # compaction.json with mcp_hang_dwell_seconds.
        mcp_hang_on = bool(cfg.get("mcp_hang_recovery_enabled"))
        mcp_hang_dwell = max(60, int(cfg.get("mcp_hang_dwell_seconds", 300) or 300))
