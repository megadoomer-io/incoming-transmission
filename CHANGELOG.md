...
- **An opt-in sweep that auto-clears a BUSY bridge session hung on an MCP tool
  ...
  manual recovery today is sending Esc to the pane. Detection requires **all three**
  signals (never bare wall-clock, so a genuinely-progressing long task is never
  cut): an **undrained inbox**, the transcript's **last `tool_use` has no matching
  `tool_result`**, AND **no transcript progress** (new assistant turn / tool_result;
  `queue-operation`-style appends don't count) for a dwell. Recovery is the SAME
  safe action as the native wedge sweep: send `Escape` (only ever cancels a tool
  call, never approves), inject a trusted inbox note, nudge a drain, and post a 🪤
  alert to the topic. In-memory episode tracking (like the wedge sweep) re-arms a
  clean episode on router restart. **Opt-in, default off** (`mcp_hang_recovery_enabled`)
  because issue #18 is **owner-deferred** — the human decides when to enable it; the
  dwell reuses the native wedge's grace (`mcp_hang_dwell_seconds`, defaults to
  `wedge_dwell_seconds`). Pure scan is `_scan_outstanding_tool` and pure policy is
