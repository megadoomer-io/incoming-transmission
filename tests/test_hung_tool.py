...
def _assistant_tool_use(tool_id, ts, name="mcp__gmail__send"):
...
def test_scan_outstanding_when_no_result(tmp_path):
    ...
    outstanding, name, last_ts = mod._scan_outstanding_tool(p)
    assert outstanding == "toolu_A"
    assert name == "mcp__gmail__send"           # name surfaced for the MCP gate
    assert last_ts == "t1"
...
def test_scan_answered_tool_is_not_outstanding(tmp_path):
    ...
    assert name is None                          # no outstanding tool -> no name
    assert last_ts == "t2"
...
def test_scan_skips_malformed_lines(tmp_path):
    ...
    p.write_text("not json\n"
                 ...
                 + "{ broken\n")
    outstanding, name, last_ts = mod._scan_outstanding_tool(str(p))
    assert outstanding == "toolu_A" and last_ts == "t1"


def test_scan_missing_file():
    mod = _load_router()
    assert mod._scan_outstanding_tool("/no/such/transcript.jsonl") == (None, None, None)
...
def test_detect_non_mcp_tool_never_hung(tmp_path):
    mod = _load_router()
    # A long-running NON-MCP tool (Bash build, WebFetch, ...) is outstanding with no
    # transcript progress and an undrained inbox — the exact shape that WOULD fire if
    # the gate were bare wall-clock. The MCP-only gate must spare it: a healthy long
    # task is never Esc-cancelled.
    tp = _write_transcript(tmp_path, [_assistant_tool_use("toolu_BUILD", "t1",
                                                          name="Bash")])
    reg = _reg(tmp_path, tp)
    _, seen = mod.detect_hung_tool(reg, {}, now_mono=1000, dwell_s=300,
                                   undrained=lambda r: 2)
    assert "100" not in seen                      # non-MCP -> episode never armed
    action, seen2 = mod.detect_hung_tool(reg, seen, now_mono=10_000, dwell_s=300,
                                         undrained=lambda r: 2)
    assert action is None                         # still never flagged, even past dwell
...
def test_shell_skips_when_auq_in_flight(monkeypatch, tmp_path):
    mod = _load_router()
    _shell_env(mod, monkeypatch, tmp_path)
    tp = _write_transcript(tmp_path, [_assistant_tool_use("toolu_A", "t1")])
    reg = _reg(tmp_path, tp)
    # The bridge's own AskUserQuestion (telegram-auq-mcp) writes auq-pending.json and
    # is a SUPPORTED outstanding tool call blocking on the owner — not a hang. Its
    # ~1700s timeout dwarfs the 300s hung-tool dwell, so without this skip an
    # undrained inbox would cancel the owner's own question ~23m early.
    (tmp_path / "sessions" / "100" / "auq-pending.json").write_text("{}")
    keys = []
    monkeypatch.setattr(mod, "_pane_for_claude_pid", lambda pid: "%5")
    monkeypatch.setattr(mod, "_tmux", lambda *a: keys.append(a) or "")
    monkeypatch.setattr(mod, "undrained_count", lambda r: 2)
    mod.handle_hung_tool(reg, "100", chat_id=42, dwell_s=300)
    assert keys == []                              # don't cancel a live AUQ
