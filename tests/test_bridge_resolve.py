...
# --- resolve() top-level entry (pane option, then spawn env; NO cwd fallback) ---
# Post-cutover, binding is programmatic on every path, so resolve() is pane-keyed
# only — the migration cwd fallback is gone. resolve() delegates to resolve_topic.

def test_resolve_returns_pane_tid():
    env = {"TMUX_PANE": "%7"}
    assert bridge_resolve.resolve(env=env, pane_option_reader=_reader({"%7": "2103"})) == "2103"


def test_resolve_returns_env_tid_in_spawn_race():
    # Pane option not yet stamped; the spawn env binding resolves it.
    env = {"TMUX_PANE": "%7", "TELEGRAM_BRIDGE_THREAD_ID": "2103"}
    assert bridge_resolve.resolve(env=env, pane_option_reader=_reader({})) == "2103"


def test_resolve_none_when_unbound():
    # No pane option and no env binding -> not a bridge session. Crucially, a cwd
    # is irrelevant now: there is no registry/cwd consultation to fall back to.
    assert bridge_resolve.resolve(env={"PWD": "/some/repo"}, pane_option_reader=_reader({})) is None
